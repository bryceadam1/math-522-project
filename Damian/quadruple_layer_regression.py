...
for features_used in num_features_tested:
    model = nn.Sequential(nn.Linear(features_used, 100), nn.ReLU(), nn.Linear(100, 100), nn.ReLU(), nn.Linear(100, 100), nn.ReLU(), nn.Linear(100, 1))

    used_training_xs = training_xs[:,:features_used]
    used_testing_xs = testing_xs[:,:features_used]

    for _ in range(10000):
        opt = optim.Adam(model.parameters(), lr = 1e-3 / features_used) #I make the learning rate smaller with more features so it doesn't diverge.
